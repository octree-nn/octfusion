...
def sdf_to_mesh(sdf, level=0.02, color=None, render_all=False):
    ...
    for i in range(nimg_to_render):
        ...
        verts_i = verts_i / n_cell - .5    # 把marching cube得到的mesh的顶点坐标归一化到[-0.5, 0.5]
...
def sdf_to_mesh_dualoctree(sdfs, level=0, bbmin=-0.9, bbmax=0.9, mesh_scale=1.0, color=None, render_all=False):
    # device='cuda'
    device=sdfs.device
    ...
    for i in range(nimg_to_render):
        sdf_i = sdfs[i].detach().cpu().numpy()

        # 两种marching cube的方式，第一种为使用dualoctree代码中的skimage.measure.marching_cubes
        # verts_i, faces_i = np.zeros((0, 3)), np.zeros((0, 3))
        # try:
        #     verts_i, faces_i, _, _ = skimage.measure.marching_cubes(sdf_i, level)   # marching cude得到vtx和faces,这里的verts_i的坐标都是在[0, resolution]上。
        # except:
        #     pass
        # if verts_i.size == 0 or faces.size == 0:
        #     print('Warning from marching cubes: Empty mesh!')
        #     return

        # verts_i = verts_i * ((bbmax - bbmin) / size) + bbmin  # [0,sz]->[bbmin,bbmax]  把vertex放缩到[bbmin, bbmax]之间
        # verts_i = verts_i * mesh_scale                        # 然后存储的时候进行放缩：mesh_scale = point_scale = 0.5

        # 第二种方式为使用SDFusion中的mcubes.marching_cubes
        verts_i, faces_i = mcubes.marching_cubes(sdf_i, level)
        verts_i = verts_i / size - .5
...
def render_sdf_dualoctree(mesh_renderer, sdfs, level=0., bbmin=-0.9, bbmax=0.9, mesh_scale=1.0, color=None, render_imsize=256, render_all=True):
    ...
    p3d_mesh = sdf_to_mesh_dualoctree(sdfs, level, bbmin, bbmax, mesh_scale, color=color, render_all=render_all)
