...
class DiffusionUNet(nn.Module):
    ...
    def forward(self, x, doctree_in, doctree_out, t, c_concat: list = None, c_crossattn: list = None):
        # x: should be latent code. shape: (bs X z_dim X d X h X w)

        if self.conditioning_key == 'None':
            out = self.diffusion_net(x, doctree_in, doctree_out, t)
            ...
            cc = c_crossattn[0]
            out = self.diffusion_net(x, doctree_in, doctree_out, t, y=cc)
        else:
