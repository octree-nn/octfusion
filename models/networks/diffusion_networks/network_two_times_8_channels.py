...
from .graph_ounet_two_t_8_channels_full_vox import UNet3DModel
...
class DiffusionUNet(nn.Module):
    ...
    def forward(self, x_small, x_large, doctree_in, t1, t2, c_concat: list = None, c_crossattn: list = None):
        # x: should be latent code. shape: (bs X z_dim X d X h X w)

        if self.conditioning_key == 'None':
            out = self.diffusion_net(x_small, x_large, doctree_in, timesteps1 = t1, timesteps2 = t2)
