...
from .graph_unet_two_t_8_channels_lr import UNet3DModel
...
class DiffusionUNet(nn.Module):
    ...
    def forward(self, x_small, x_feature, doctree_in, t1, t2, c_concat: list = None, c_crossattn: list = None):
        # x: should be latent code. shape: (bs X z_dim X d X h X w)

        if self.conditioning_key == 'None':
            out = self.diffusion_net(x_small, x_feature, doctree_in, timesteps1 = t1, timesteps2 = t2)
