...
class DiffusionUNet(nn.Module):
    ...
    def forward(self, x, doctree_in, doctree_out, t1, t2, t3, c_concat: list = None, c_crossattn: list = None):
        # x: should be latent code. shape: (bs X z_dim X d X h X w)

        if self.conditioning_key == 'None':
            out = self.diffusion_net(x, doctree_in, doctree_out, timesteps1 = t1, timesteps2 = t2, timesteps3 = t3)
