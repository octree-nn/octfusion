...
class SDFusionModel(BaseModel):
    ...
    def forward(self):
        ...
        output = self.df(x_feature = noised_feature, doctree = self.doctree_in, timesteps = noise_level, label = self.label)
    ...
    def inference(self, data, ema = False, ddim_steps=200, ddim_eta=0., phase = ""):
        ...
        for time, time_next in feature_iter:
            ...
            if ema:
                pred_noise = self.ema_df(x_feature = noised_feature, doctree = doctree_small, timesteps = noise_cond, label = self.label)
            else:
                pred_noise = self.df(x_feature = noised_feature, doctree = doctree_small, timesteps = noise_cond, label = self.label)
