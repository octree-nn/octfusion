...
from models.networks.diffusion_networks.network_two_times_8_channels import DiffusionUNet
...
class SDFusionModel(BaseModel):
    ...
    def forward(self):
        ...
        output = self.df(x_small = noised_split_small, x_large = noised_split_large, doctree_in = self.doctree_in, t1 = times1, t2 = times2)
    ...
    @torch.no_grad()
    def uncond(self, batch_size=16, category = 'airplane', ema = True, ddim_steps = 200, ddim_eta = 0., save_index = 0):
        ...
        timesteps = ddim_timesteps
        total_steps = timesteps.shape[0]
        small_time_range = np.flip(timesteps)
        small_iterator = tqdm(small_time_range, desc='Small Stage DDIM Sampler', total=total_steps)

        for i, step in enumerate(small_iterator):
            index = total_steps - i - 1
            time1 = torch.full((batch_size,), step, device=self.device, dtype=torch.long)
            time2 = torch.zeros((batch_size,), device = self.device).long() + self.num_timesteps
            if ema:
                e_t = self.ema_df(x_small = noised_split_small, x_large = None, doctree_in = None, t1 = time1, t2 = time2)
            else:
                e_t = self.df(x_small = noised_split_small, x_large = None, doctree_in = None, t1 = time1, t2 = time2)
            ...
            pred_x0 = (noised_split_small - sqrt_one_minus_at * e_t) / a_t.sqrt()
            ...
            dir_xt = (1. - a_prev - sigma_t**2).sqrt() * e_t
            noise = sigma_t * torch.randn(noised_split_small.shape, device = self.device)
            noised_split_small = a_prev.sqrt() * pred_x0 + dir_xt + noise


        print(noised_split_small.max(), noised_split_small.min())
        print(noised_split_small.shape)
        ...
        total_steps = timesteps.shape[0]
        large_time_range = np.flip(timesteps)
        large_iterator = tqdm(large_time_range, desc='Large Stage DDIM Sampler', total=total_steps)

        leaf_num = doctree_small.lnum[self.full_depth:self.small_depth].sum()

        for i, step in enumerate(large_iterator):
            index = total_steps - i - 1
            time1 = torch.zeros((batch_size,), device = self.device).long()
            time2 = torch.full((batch_size,), step, device=self.device, dtype=torch.long)
            if ema:
                e_t = self.ema_df(x_small = None, x_large = noised_split_large, doctree_in = doctree_small, t1 = time1, t2 = time2)
            else:
