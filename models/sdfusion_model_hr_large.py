...
from models.networks.diffusion_networks.ocnn_unet_hr_large import UNet3DModel
...
class SDFusionModel(BaseModel):
    ...
    def set_input(self, input=None):
        ...
        self.split_large = input['split_large'].to(self.device)
        self.octree_in = self.split2octree_small(self.split_small)
    ...
    def forward(self):
        ...
        output = self.df(x_large = noised_split_large, octree = self.octree_in, timesteps = noise_level)
    ...
    def inference(self, data, ema = False, ddim_steps=200, ddim_eta=0., phase = 'train'):
        ...
        for time, time_next in large_iter:
            ...
            if ema:
                x_large_start = self.ema_df(x_large = noised_large, octree = octree_small, timesteps = noise_cond)
            else:
                x_large_start = self.df(x_large = noised_large, octree = octree_small, timesteps = noise_cond)
