...
from models.networks.diffusion_networks.network_large import DiffusionUNet
...
class SDFusionModel(BaseModel):
    ...
    def forward(self):
        ...
        output = self.df(x_large = noised_split_large, doctree = self.doctree_in, t = times)
    ...
    def uncond(self, data, split_path, category = 'airplane', ema = True, ddim_steps = 200, ddim_eta = 0., save_index = 0):
        ...
        for i, step in enumerate(iterator):
            ...
            if ema:
                e_t = self.ema_df(x_large = noised_split_large, doctree = doctree_small, t = time)
            else:
                e_t = self.df(x_large = noised_split_large, doctree = doctree_small, t = time)
